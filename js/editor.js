...
let tool = 'road'; // 'road', 'finish', 'erase'
...
// Resolve tile for new cells: use presence mask, pick best pair if 3+
...
function resolveCell( gx, gz ) {
	...
	if ( ! cell.mesh ) {
		...
		// Existing cell: check if both exits are already connected.
		// If so, don't change — avoid breaking existing connections.
		const currentExits = getCellExits( cell );
		const cMask = getConnectivityMask( gx, gz );
		const connectedExits = currentExits & cMask;
		const connectedCount = ( connectedExits >> 3 & 1 ) + ( connectedExits >> 2 & 1 )
			+ ( connectedExits >> 1 & 1 ) + ( connectedExits & 1 );

		if ( connectedCount >= 2 ) return; // fully connected, don't change

		// Has a free exit — re-resolve to pick up new connection
...
function placeRoad( gx, gz, isFinish ) {

	const key = cellKey( gx, gz );

	// If placing a finish, remove finish flag from the old finish cell
	if ( isFinish ) {
	...
	grid.set( key, { type: 'track-straight', orient: 0, isFinish: !! isFinish, mesh: null } );
	resolveCellAndNeighbors( gx, gz );
	save();
...
function updateGhost( gx, gz ) {
	...
	const isFinish = ( tool === 'finish' );
	const ghostCell = { type: 'track-straight', orient: 0, isFinish, mesh: null };
	grid.set( key, ghostCell );

	// Resolve ghost: connect to neighbors, pick best pair if 3+
	const [ baseType, orient ] = resolveNewTile( gx, gz );
	const type = ( isFinish && baseType === 'track-straight' ) ? 'track-finish' : baseType;
	...
	for ( const [ nx, nz ] of neighbors ) {
		...
		// Skip neighbors that are already fully connected
		const nExits = getCellExits( nCell );
		const nConn = getConnectivityMask( nx, nz );
		const connected = nExits & nConn;
		const connCount = ( connected >> 3 & 1 ) + ( connected >> 2 & 1 )
			+ ( connected >> 1 & 1 ) + ( connected & 1 );
...
function loadSaved() {
	...
	try {
		...
		for ( const [ gx, gz, type, orient ] of arr ) {
			...
			trackGroup.add( mesh );
			grid.set( cellKey( gx, gz ), { type, orient, isFinish, mesh } );
...
function selectTool( t ) {
	...
	btnFinish.classList.toggle( 'active', t === 'finish' );
	btnErase.classList.toggle( 'active', t === 'erase' );

}

btnRoad.addEventListener( 'click', () => selectTool( 'road' ) );
btnFinish.addEventListener( 'click', () => selectTool( 'finish' ) );
...
function handleDraw( clientX, clientY ) {
	...
	if ( isErasing ) {
		...
		placeRoad( cell.gx, cell.gz, tool === 'finish' );
...
renderer.domElement.addEventListener( 'mousedown', ( e ) => {
	...
	// Middle mouse, ctrl+left, or space+left for panning
